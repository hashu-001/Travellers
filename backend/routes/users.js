...
// Mark current user as active (called periodically from the client)
...
// Mark current user as offline (called on explicit logout)
router.post('/offline', auth, async (req, res) => {
  try {
    await User.updateOne({ _id: req.user._id }, { lastSeenAt: null });
    res.json({ ok: true });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
